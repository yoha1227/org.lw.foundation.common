...
import org.hy.foundation.utils.data.DateUtil;
...
public class PropertyFilter {
	 ...
	 * 值类型分为：
	 * <pre>
	 * S(String.class), I(Integer.class), L(Long.class), B(Boolean.class), D(Date.class),C(Collection.class);
	 * </pre>
	 * 
	 * 比较方式分为：
	 * <pre>
	...
	public static List<PropertyFilter> parsePropertyFilterExp(String exp) {
		...
		exp=exp+",";
		String expReg="([^\\_]*)_([SILBD]C?)_([^\\_]*)_([^\\_]*)_(.*?)_(.*?),";
		...
		for(String[] singleExp : exps){
			...
			if(mt.equals("ISNULL")){
				...
				pv=DateUtil.parseDate(singleExp[3]);
